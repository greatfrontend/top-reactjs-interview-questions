...
The React team ships a codemod that automatically converts existing `forwardRef()` usages to the new prop form. `forwardRef()` itself still works for now and is expected to be deprecated, then removed, in a future major.
