...
A common claim you will see is "the virtual DOM is faster than the real DOM." This is mostly a myth. Doing extra JavaScript work to diff trees and then update the DOM cannot be faster than a hand-optimized direct DOM update. What React actually gives you is a _good-enough_, _predictable_ way to produce correct DOM updates from a declarative description, without having to write that orchestration yourself. Speed relative to hand-written imperative code is a side effect, not the goal.
...
- [Dan Abramov — "React as a UI Runtime"](https://overreact.io/react-as-a-ui-runtime/)
